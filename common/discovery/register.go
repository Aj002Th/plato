...
func (s *ServiceRegister) UpdateValue(val *EndpointInfo) error {
	...
	s.val = value
	log.Printf("ServiceRegister.updateValue leaseID=%d Put key=%s,val=%s, success!", s.leaseID, s.key, s.val)
	return nil
}
...
func (s *ServiceRegister) ListenLeaseRespChan() {
	for leaseKeepResp := range s.keepAliveChan {
		log.Printf("lease success leaseID:%d, Put key:%s,val:%s reps:+%v",
			s.leaseID, s.key, s.val, leaseKeepResp)
	}
	log.Printf("lease failed !!!  leaseID:%d, Put key:%s,val:%s", s.leaseID, s.key, s.val)
}
...
func (s *ServiceRegister) Close() error {
	...
	log.Printf("lease close !!!  leaseID:%d, Put key:%s,val:%s  success!", s.leaseID, s.key, s.val)
	return s.cli.Close()
}
